  ...
  return (
    <>
      <nav className="pointer-events-none fixed bottom-0 left-1/2 z-40 w-full max-w-[480px] -translate-x-1/2 px-3 pb-3 safe-bottom">
        <div className="pointer-events-auto glass shadow-floating mx-auto flex items-center justify-around rounded-full border border-border/40 px-2 py-1.5">
          {items.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex flex-col items-center justify-center gap-0.5 rounded-full px-4 py-1.5 transition-all duration-300",
                  isActive
          ...
          <button
            ...
            className="flex flex-col items-center justify-center gap-0.5 rounded-full px-4 py-1.5 text-muted-foreground transition-colors hover:text-foreground"
            aria-label="Menu"
          >
